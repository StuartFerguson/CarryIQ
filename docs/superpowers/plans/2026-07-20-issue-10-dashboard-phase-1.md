# Issue 10 Dashboard Phase 1 Implementation Plan
...
### Task 1: Add dashboard projection models and queries
...
```bash
git add src/CarryIQ.Domain src/CarryIQ.Infrastructure tests
git commit -m "feat: add dashboard projection models"
```

### Task 2: Add the dashboard view-model and shell wiring
...
**Interfaces:**
- Consumes: dashboard projection query service, `IPracticeSessionRepository`, `IShotRepository`, `IGolferProfileRepository` or equivalent profile-loading path already in the app
...
```csharp
public sealed class DashboardViewModel : ObservableObject, IShellScreenViewModel
{
    public ObservableCollection<DashboardMetricCardViewModel> MetricCards { get; }
    public ObservableCollection<RecentSessionRowViewModel> RecentSessions { get; }
    public RecentSessionRowViewModel? SelectedSession { get; set; }
...
```bash
git add src/CarryIQ.App tests/CarryIQ.UnitTests/App
git commit -m "feat: add dashboard view model"
```
...
```bash
git add src/CarryIQ.App tests/CarryIQ.UnitTests/App
git commit -m "feat: add dashboard phase 1"
```
